...
function isEntitySchema(schema) {
    return (
        ...
        schema instanceof CompositeEntitySchema
    )
}
...
> extends EntitySchema<A> {
    ...
    normalize(data: unknown, entities?: Object = {}): NormalizeState {
        ...
        let {schemas, result: mainResult} = super.normalize(
            adjustedData,
            entities,
        )
    ...
    denormalize(
        ...
        const mainDenormalizedState = super.denormalize(
            {result: result[name], entities},
            path,
        )
