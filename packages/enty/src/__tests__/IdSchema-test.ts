...
test("denormalize is almost the inverse of normalize", () => {
    ...
    expect(data.foo).toEqual(
        fooValues.denormalize(fooValues.normalize(data)).foo.id,
    )
})
...
test("normalize", () => {
    ...
    expect(data).toEqual(
        new IdSchema(foo).normalize("1", entities).entities.foo["1"],
    )
    expect(data).toEqual(
        new IdSchema(foo).normalize("1", undefined).entities.foo["1"],
    )
})
...
test("denormalize", () => {
    ...
    expect(data).toEqual(
        fooValue.denormalize({result: "1", entities}, undefined),
    )
})